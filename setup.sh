...
# ── Step 12: Fix CuDNN + Train ────────────────────────────────────────────────
print("\n[Step 12] Resolving CuDNN version...")
subprocess.run([sys.executable, '-m', 'pip', 'install', '-q',
                'nvidia-cudnn-cu12>=9.3,<10'], check=True)
cudnn_lib = subprocess.check_output(
    [sys.executable, '-c',
     'import nvidia.cudnn, os; print(os.path.dirname(nvidia.cudnn.__file__))'],
    text=True).strip() + '/lib'
print(f"  \u2192 CuDNN lib path: {cudnn_lib}")
...
train_env = {
    **os.environ,
    'LD_LIBRARY_PATH': f"{cudnn_lib}:{os.environ.get('LD_LIBRARY_PATH', '')}",
