...
if USING_REAL:
    ...
    print(f"   Target: {SYNTHETIC_COUNT} synthetic + {REAL_TARGET} augmented real = {NUM_SAMPLES} total")
else:
    print(f"📢 No real recordings found — generating {NUM_SAMPLES} synthetic samples.")
    print(f"   (Place .wav/.mp3/.m4a files in {REAL_DIR}/ to use real recordings)")
...
if USING_REAL:
    ...
    subprocess.run([sys.executable, '-m', 'pip', 'install', '-q', 'pydub', 'soundfile', 'scipy'],
                   check=True)
    ...
    if len(raw_clips) == 0:
        print("  ⚠️  No valid clips detected — check silence gaps in recordings.")
        print("  ⚠️  Falling back to synthetic-only training.")
        USING_REAL = False
    else:
        reps_needed = max(1, -(-REAL_TARGET // len(raw_clips)))  # ceiling division
        augmented_count = 0
        ...
        print(f"✅ {total_samples} total samples in generated_samples/ ({synth_count} synthetic + {augmented_count} real)")
