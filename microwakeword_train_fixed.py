...
def validate_nonstreaming(config, data_processor, model, test_set):
    ...
    if data_processor.get_mode_size("validation_ambient") > 0:
        ...
        # both test sets
        all_true_positives = ambient_predictions["tp"].numpy() if hasattr(ambient_predictions["tp"], 'numpy') else ambient_predictions["tp"]
        ambient_false_positives = (ambient_predictions["fp"].numpy() if hasattr(ambient_predictions["fp"], 'numpy') else ambient_predictions["fp"]) - test_set_fp
        all_false_negatives = ambient_predictions["fn"].numpy() if hasattr(ambient_predictions["fn"], 'numpy') else ambient_predictions["fn"]
